...
// ============================================================
// MODULES — try/catch bilan (biri buzilsa ham qolganlari ishlaydi)
// ============================================================
function safeRequire(modPath, fallbackName) {
  try {
    return require(modPath);
  } catch (e) {
    console.error(`❌ Modul yuklanmadi: ${fallbackName || modPath}`);
    console.error(`   Sabab: ${e.message}`);
    if (e.code === 'ERR_REQUIRE_ASYNC_MODULE') {
      console.error(`   → ${modPath} faylida top-level await bor. Uni funksiya ichiga ko'chiring.`);
    }
    throw e;
  }
}
...
server.listen(PORT, HOST, () => {
  ...
  console.log('  ' + line);
  console.log(`  Frontend:   http://localhost:${PORT}/frontend/pages/main.html`);
  ...
  console.log('  ' + line);
  try {
    console.log(`  Data file:  ${require('./utils/db').DATA_PATH}`);
  } catch (_) {}
  console.log('  ' + line);
